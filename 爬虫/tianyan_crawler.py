...
class GetCookies:
    ...
    @staticmethod
    def compare_images(image_be, image_af, i: int, j: int, threshold: int = 50) -> bool:
        """
...
class GetCompanyInfo:
    ...
    def __init__(self, company_name, cookies_dict):
        """
        初始化参数
    ...
    def get_company_info(self):
        ...
        headers = self.headers
        headers['path'] = f"/company/{company_url.split('/')[-1]}"
...
if __name__ == '__main__':
    cookies = GetCookies('your_account', 'password').get_cookies()
    com_data = GetCompanyInfo('阿里巴巴', cookies).get_company_info()
    print(com_data)
