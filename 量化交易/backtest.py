...
class ExchangeAPI:
    def __init__(self, data: pd.DataFrame, cash: float, commission: float) -> None:
        assert_msg(0 < cash, '初始金额数量大于0，输入的现金数量：{}'.format(cash))
...
class Backtest:
    ...
    def __init__(self,
        ...
        # 验证OHLC数据格式
        assert_msg(len(data.columns.intersection({'Open', 'High', 'Low', 'Close', 'Volume'}) == 5),
